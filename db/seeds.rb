...
angry_squid_def = Definition.create!(user: User.first, content: "'<%= %>': angry squid shoots ink or evaluates ruby code and prints", word: angry_squid)
controller_def = Definition.create!(user: User.first, content: "Communicates between the model (database) and views (user interface/website/browser)", word: controller)
controller_def_2 = Definition.create!(user: User.first, content: "A controller is the waiter; between the model and the view. It determines what the user will be doing/seeing based on the request", word: controller)
ice_cream_cone_def = Definition.create!(user: User.first, content: "'<% %>': Ice cream cone evaluates ruby code but does not print", word: ice_cream_cone)
model_def = Definition.create!(user: User.first, content: "A blueprint for creating instances; manages data in the database", word: model)
module_word_def_js = Definition.create!(user: User.first, content: "Your personal toolbox of functions that can be called upon instead of repeating them in your code", word: module_word)
module_word_def_ruby = Definition.create!(user: User.first, content: "Your personal toolbox of methods that can be called upon instead of repeating them in your code", word: module_word)
mvc_def = Definition.create!(user: User.first, content: "Model-View-Controller, its a design pattern for building web applications  (chef, waiter, serving/dining table)", word: mvc)
params_def = Definition.create!(user: User.first, content: "A hash with all the spicy user's input from the url input field or forms", word: params)
equest_def = Definition.create!(user: User.first, content: "Client sends a requests to get information to/from the server", word: request)
response_def = Definition.create!(user: User.first, content: "What server sends back to the user (client) after it processes the data", word: response)
route_def = Definition.create!(user: User.first, content: "A route is a path that allows you access a page of your preference; it connects with an appropriate method and renders an appropriate view", word: route)
strong_params_def = Definition.create!(user: User.first, content: "Rails safeguard against the meanies of the world -- it's a nested hash", word: strong_params)
view_def = Definition.create!(user: User.first, content: "The HTML files (what is displayed on the browser)", word: view)
